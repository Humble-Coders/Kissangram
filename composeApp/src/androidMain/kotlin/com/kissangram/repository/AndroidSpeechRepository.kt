...
class AndroidSpeechRepository(
...
) : com.kissangram.repository.SpeechRepository {
    ...
    override fun isAvailable(): Boolean {
        ...
        if (!android.speech.SpeechRecognizer.isRecognitionAvailable(context)) {
            android.util.Log.w("AndroidSpeechRepository", "Speech recognition not available on this device")
            return false
        }
        ...
        if (activities.isEmpty()) {
            android.util.Log.w("AndroidSpeechRepository", "No speech recognition activities found")
            return false
        }
    ...
    override suspend fun startListening(): String {
        ...
        if (isListening) {
            android.util.Log.d("AndroidSpeechRepository", "Already listening, skipping start")
            return ""
        }
    ...
    private suspend fun useImprovedSpeechRecognizer(): String = suspendCancellableCoroutine { continuation ->
        try {
            ...
            android.util.Log.e("AndroidSpeechRepository", "Exception: ${e.message}", e)
            try {
    ...
    private fun startRecognizer(continuation: kotlin.coroutines.Continuation<String>) {
        try {
            ...
            if (recognizer == null) {
                android.util.Log.e("AndroidSpeechRepository", "Failed to create SpeechRecognizer")
                try {
            ...
            val listener = object : android.speech.RecognitionListener {
                override fun onReadyForSpeech(params: android.os.Bundle?) {
                    android.util.Log.d("AndroidSpeechRepository", "Ready for speech")
                }
                
                override fun onBeginningOfSpeech() {
                    android.util.Log.d("AndroidSpeechRepository", "Beginning of speech")
                }
                ...
                override fun onEndOfSpeech() {
                    android.util.Log.d("AndroidSpeechRepository", "End of speech")
                ...
                override fun onError(error: Int) {
                    val errorMsg = when (error) {
                        ...
                        android.speech.SpeechRecognizer.ERROR_CLIENT -> {
                            ...
                            android.util.Log.e("AndroidSpeechRepository", "Client error (SDK: ${Build.VERSION.SDK_INT})")
                            
                            if (retryCount < maxRetries && shouldContinueListening && isListening) {
                                retryCount++
                                android.util.Log.d("AndroidSpeechRepository", "Retrying after client error (attempt $retryCount/$maxRetries)")
                                mainHandler.postDelayed({
                                    if (shouldContinueListening && isListening) {
                                        try {
                                            ...
                                            android.util.Log.e("AndroidSpeechRepository", "Error retrying: ${e.message}")
                                        }
                                    }
                                }, 500L * retryCount) // Exponential backoff
                            } else if (!shouldContinueListening) {
                                try {
                                    continuation.resumeWithException(Exception("Client error"))
                                } catch (e: Exception) {
                                    // Continuation already completed
                                }
                            }
                        ...
                        android.speech.SpeechRecognizer.ERROR_NO_MATCH -> {
                            ...
                            if (shouldContinue) {
                                ...
                                android.util.Log.d("AndroidSpeechRepository", "No match but stopping - shouldStop: $shouldStopAfterCurrent")
                                try {
                                    continuation.resumeWithException(Exception("No match"))
                                } catch (e: Exception) {
                                    // Continuation already completed
                                }
                            }
                        ...
                        android.speech.SpeechRecognizer.ERROR_SPEECH_TIMEOUT -> {
                            ...
                            if (shouldContinue) {
                                ...
                                android.util.Log.d("AndroidSpeechRepository", "Timeout but stopping - shouldStop: $shouldStopAfterCurrent")
                                try {
                                    continuation.resumeWithException(Exception("Timeout"))
                                } catch (e: Exception) {
                                    // Continuation already completed
                                }
                            }
                    ...
                    android.util.Log.e("AndroidSpeechRepository", "Error: $errorMsg")
                }
                ...
                override fun onResults(results: android.os.Bundle?) {
                    ...
                    synchronized(lock) {
                        ...
                        android.util.Log.d("AndroidSpeechRepository", "=== ON RESULTS CALLED ===")
                        android.util.Log.d("AndroidSpeechRepository", "Flags check: shouldStop=$shouldStop, shouldStopAfterCurrent=$shouldStopAfterCurrent, shouldContinue=$shouldContinueListening, isListening=$isListening")
                    }
                    
                    val matches = results?.getStringArrayList(android.speech.SpeechRecognizer.RESULTS_RECOGNITION)
                    if (matches != null && matches.isNotEmpty()) {
                        val recognizedText = matches[0]
                        android.util.Log.d("AndroidSpeechRepository", "Recognized: $recognizedText")
                    ...
                    if (shouldStop || shouldStopFinal) {
                        android.util.Log.d("AndroidSpeechRepository", "Final results received, NOT restarting (shouldStop: $shouldStopAfterCurrent, shouldContinue: $shouldContinueListening, isListening: $isListening)")
                        android.util.Log.d("AndroidSpeechRepository", "Final accumulated text: $accumulatedText")
                    ...
                    if (finalCheck) {
                        android.util.Log.d("AndroidSpeechRepository", "Final check before restart - stopping instead")
                        android.util.Log.d("AndroidSpeechRepository", "Final accumulated text: $accumulatedText")
                    ...
                    android.util.Log.d("AndroidSpeechRepository", "Final results received, continuing to listen")
                    restartListening(recognizer, this)
                }
                ...
                override fun onPartialResults(partialResults: android.os.Bundle?) {
                    ...
                    if (matches != null && matches.isNotEmpty()) {
                        ...
                        android.util.Log.d("AndroidSpeechRepository", "Partial: $partialText")
            ...
            try {
                ...
                android.util.Log.d("AndroidSpeechRepository", "Started listening (SDK: ${Build.VERSION.SDK_INT})")
            } catch (e: Exception) {
                android.util.Log.e("AndroidSpeechRepository", "Error starting: ${e.message}", e)
                recognizer.destroy()
                speechRecognizer = null
                try {
    ...
    private fun restartListening(recognizer: android.speech.SpeechRecognizer, listener: android.speech.RecognitionListener) {
        ...
        if (shouldStop) {
            android.util.Log.d("AndroidSpeechRepository", "Skipping restart - shouldStop: $shouldStopAfterCurrent, shouldContinue: $shouldContinueListening, isListening: $isListening")
            return
        }
        ...
        restartRunnable = Runnable {
            ...
            if (shouldStopNow) {
                android.util.Log.d("AndroidSpeechRepository", "Cancelled restart - flags changed")
                return@Runnable
            }
            ...
            try {
                ...
                recognizer.startListening(intent)
                android.util.Log.d("AndroidSpeechRepository", "Restarted listening")
            } catch (e: Exception) {
                android.util.Log.e("AndroidSpeechRepository", "Error restarting: ${e.message}")
                ...
                if (retryCount < maxRetries) {
                    ...
                    try {
                        ...
                        android.util.Log.e("AndroidSpeechRepository", "Error recovering: ${recoveryError.message}")
                        isListening = false
    ...
    override suspend fun stopListening() {
        android.util.Log.d("AndroidSpeechRepository", "=== STOP LISTENING CALLED ===")
        android.util.Log.d("AndroidSpeechRepository", "Before: shouldStop=$shouldStopAfterCurrent, shouldContinue=$shouldContinueListening, isListening=$isListening")
        
        // Synchronized flag setting to prevent race conditions
        synchronized(lock) {
            shouldStopAfterCurrent = true
            shouldContinueListening = false
            android.util.Log.d("AndroidSpeechRepository", "After setting flags: shouldStop=$shouldStopAfterCurrent, shouldContinue=$shouldContinueListening, isListening=$isListening")
        }
        ...
        restartRunnable?.let { 
            ...
            android.util.Log.d("AndroidSpeechRepository", "Cancelled pending restart")
        }
        ...
        if (recognizer != null) {
            try {
                ...
                android.util.Log.d("AndroidSpeechRepository", "Stopped recognizer (waiting for final results)")
            } catch (e: Exception) {
                android.util.Log.e("AndroidSpeechRepository", "Error stopping: ${e.message}")
                // If stop fails, cancel it
                try {
                    recognizer.cancel()
                } catch (cancelEx: Exception) {
                    android.util.Log.e("AndroidSpeechRepository", "Error cancelling: ${cancelEx.message}")
                }
            }
        }
        ...
        stopTimeoutRunnable = Runnable {
            synchronized(lock) {
                if (shouldStopAfterCurrent && isListening) {
                    android.util.Log.d("AndroidSpeechRepository", "Timeout reached, forcing cleanup")
                    actuallyStopListening()
                }
            }
        }
    ...
    private fun actuallyStopListening() {
        android.util.Log.d("AndroidSpeechRepository", "Actually stopping listening")
        ...
        if (recognizer != null) {
            try {
                ...
                android.util.Log.e("AndroidSpeechRepository", "Error cancelling: ${e.message}")
            }
            ...
            try {
                ...
                android.util.Log.e("AndroidSpeechRepository", "Error destroying: ${e.message}")
            }
        }
