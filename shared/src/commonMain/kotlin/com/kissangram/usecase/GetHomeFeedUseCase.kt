...
class GetHomeFeedUseCase(
    ...
    suspend operator fun invoke(page: Int = 0, pageSize: Int = 20, forceRefresh: Boolean = false): List<Post> {
        require(page >= 0) { "Page must be non-negative" }
        require(pageSize in 1..50) { "Page size must be between 1 and 50" }
        
        return feedRepository.getHomeFeed(page, pageSize, forceRefresh)
    }
}
